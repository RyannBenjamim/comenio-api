...
import { 
  resolvePostContext,
  resolveRespostaContext,
  resolveCurtidaContext
} from '../utils/resolveSocialContext'

import { AuthenticatedRequest } from '../../../../common/interfaces/AuthenticatedRequest';
import { extractUUID } from '../../../../common/utils/extractUUID';

@Injectable()
export class SocialContextResolverGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (postId) {
      const contextData = await resolvePostContext(this.prisma, postId, user.instituicaoId);
      if (!contextData) throw new NotFoundException('Post não encontrado.');
    ...
    if (respostaId) {
      const contextData = await resolveRespostaContext(this.prisma, respostaId, user.instituicaoId);
      if (!contextData) throw new NotFoundException('Resposta não encontrada.');
      req.context.feedId = contextData.feedId ?? undefined;
      req.context.comunidadeId = contextData.comunidadeId ?? undefined;
      return true;
    }
    ...
    if (curtidaId) {
      const contextData = await resolveCurtidaContext(this.prisma, curtidaId, user.instituicaoId);
      if (!contextData) throw new NotFoundException('Curtida não encontrada.');
      req.context.feedId = contextData.feedId ?? undefined;
      req.context.comunidadeId = contextData.comunidadeId ?? undefined;
      return true;
    }
