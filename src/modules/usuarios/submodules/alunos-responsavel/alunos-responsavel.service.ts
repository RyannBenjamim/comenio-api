...
export class AlunosResponsavelService {
  ...
  async create(relacaoDto: RelacaoDto): Promise<AlunosResponsavel> {
    // Verifica se o responsavel e o aluno existem
    const [ existingResponsavel, existingAluno ] = await Promise.all([
      this.usuariosService.findOne(relacaoDto.responsavelId),
      this.usuariosService.findOne(relacaoDto.alunoId)
    ]);
  ...
  async findAll(responsavelId?: string, alunoId?: string): Promise<AlunosResponsavel[]> {
    let userId: string;
    let cargo: string;
    let where: any;
    ...
    const existing = await this.usuariosService.findOne(userId);
